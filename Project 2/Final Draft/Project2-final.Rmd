...
yearly_emissions_by_area <- aggregate(df_longer$'total emissions (in kilotonnes)', by = list(df_longer$year, df_longer$Area), FUN = sum)
...
```{r, echo=TRUE}
yearly_emissions <- aggregate(df_longer$'total emissions (in kilotonnes)', by=list(df_longer$year), FUN = sum)
yearly_emissions
...
```{r, echo=TRUE}
emissions_by_area <- aggregate(df_longer$'total emissions (in kilotonnes)', by = list(df_longer$Area), FUN = sum)
...
```
```{r, echo=FALSE}
knit_table(head(tbl1_permit_lvl), 'Permit Data Head') |> scroll_box(width = "100%", box_css = "border: 1px solid #FFFFFF;")
```
...
```
```{r, echo=FALSE}
knit_table(head(tbl2_permit_act_lvl), 'Permit Action Data Head') |> scroll_box(width = "100%", box_css = "border: 1px solid #FFFFFF;")
```
...
```
```{r, echo=FALSE}
knit_table(head(tbl3_facility_lvl), 'Facility Data Head')
```
...
```
```{r, echo=FALSE}
knit_table(head(tbl4_app_lvl), 'NPDES Permit Applicant Data Head')
```
...
##### View Tidy Data 
```{r, echo=FALSE}
knit_table(head(df), 'Tidy Data Head')
```
