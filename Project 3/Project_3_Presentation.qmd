...
2. Developed reference tables to store distinct categorical values (e.g., skill categories) and remove partial dependencies. 
...
4. Transformed the OES and Industry and Occupation Projection datasets from wide to  long  to make the data tidy and allow for better usability. By transforming these datasets each variable is its own column, and each observation is its own row (i.e., reducing multiple hourly and annual wage statistics columns into three columns for duration, type, and value); thus, making the data tidy.
