...
**NOTE [OLIVIA]:** the oes wage data we pulled in is specific to data science. The website verbiage   is "Industry profile for Data Scientists: a list of all industries with employment in Data Scientists
...
soc_industry_lnk <- 
  ...
  distinct()
```
create industrya and soc code reference tables
```{r}
industry_ref <- 
  soc_industry_project |>
...
### OES Data
**NOTE** this data provides an industry profile for data science meaning the data is specifically based on industries with employment in Data Scientists
```{r col-names-oes}
colnames(oes) = c(
...
```{r}
oes_df = oes |>
  # extract soc code from occupation
...
=======
>>>>>>> c4a1e4c7c2d610134304f1b95470039c2afe117f
```
```{r data-analysis-preview}
knit_table(head(ep_skills_df_clean), 'Preview Cleaned ep_skills_df')
knit_table(head(tech_skills_df_clean), 'Preview Cleaned tech_skills_df')
