...
# max_refresh_test_search_df <- test_search_df |> 
#   group_by(week, rank, term_type) |> 
#   slice(which.max(as.Date(refresh_date, '%m/%d/%Y')))
```
...
dtm_top_term <- top_term_df |> 
  count(week, top_term) |> 
  cast_dtm(document = week, term = top_term, value = n)
```
