...
**Google Search Data Cleaning and Tidying**
  ...
  - Unnest week and rank to transform dataframe into a wide-format where
...
**GDELT Event Mentions Data Cleaning**
  ...
  - Subsets the dataframe to be on the Event level by filters to the event
    mention with the highest confidence
...
mention_gdelt_df_cleaned = mention_gdelt_df |>
  mutate(
    GLOBALEVENTID = as.character(GLOBALEVENTID),
    EventTimeDate =  format(lubridate::ymd_hms(EventTimeDate),'%Y-%m-%d %H:%M:%S'),
    MentionTimeDate = format(lubridate::ymd_hms(MentionTimeDate),'%Y-%m-%d %H:%M:%S'),
    ) |>
...
ggplot(most_freq_tokens, aes(x=reorder(word, -n), y=n, fill = color_group))+
  ...
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1, color = titles),
    axis.text.y = element_text(color = titles),
...
token_str_gsearch = rising_term_df |>
  ...
  distinct() |>
  left_join(rising_term_df, by = join_by(search_id)) |>
...
token_str_gdelt = mention_gdelt_df_cleaned |>
  ...
  distinct() |>
  left_join(mention_gdelt_df_cleaned, by = join_by(GLOBALEVENTID)) |>
...
sim_df = inner_join(rising_term_df,
  ...
  select(term, refresh_date, clean_article) |>
  distinct() |>
  left_join(token_str_gsearch, by = join_by(term, refresh_date)) |>
...
sim_df |>
  ...
  theme(
    ...
    plot.subtitle = element_text(color = titles))+
  labs(
    title = 'Rising Google Search Terms & GDELT Event
      Source Levenshtein Similarity Score > 0.4',
...
ggsave('Plot_Images/sim_jw_matrix.png', width = 8,
  height = 5.6,
  units = c("in"))
```
Assessment:
