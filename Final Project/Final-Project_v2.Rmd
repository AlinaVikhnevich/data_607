...
bq_auth(path = "/Users/olivia/Downloads/data607finalproject-6c666e5b6214.json")
```
...
all_files = data_frame(
  time = (seq(ymd_hms('20250506000000'),ymd_hms('20250512000000'), by = '1 day'))
  ) |>
...
head(gdelt_df)
```
In this chunk, we load the most event mention export files and apply column
names.
...
for (i in 1:nrow(all_files)){
  print(all_files$time[i])
  download.file(all_files$file_url[i], destfile = all_files$destfile[i], mode = "wb", quiet = TRUE)
  # Unzip the file
  unzip(all_files$destfile[i], exdir='Data/GDELT')
  csv_file <- paste0('Data/GDELT/', gsub(".zip", "", basename(all_files$file_url[i])))
  # Read the CSV file with correct case-sensitive name
  if (i == 1) {
    mention_gdelt_df <- read_csv(csv_file, col_names = FALSE)
    mention_gdelt_df <- read_delim(csv_file, delim = "\t", col_names = mention_gdelt_cols, show_col_types = FALSE)
  }
  else{
    temp_df <- read_csv(csv_file, col_names = FALSE)
    temp_df <- read_delim(csv_file, delim = "\t", col_names = mention_gdelt_cols, show_col_types = FALSE)
...
**Google Search Data Cleaning and Tidying**
  ...
  - Fills Null `score` values with zero
...
**GDELT Event Data Cleaning and Tidying**
...
```{r join-tbls}
gsearch_gdelt <- left_join(long_google_search_df, 
            gdelt_df_cleaned,
...
sim_df |>
  ...
  tally() |>
  arrange(desc(n)) |>
  ggplot(aes(x=reorder(clean_term, n), y=n)) +
  mutate(color_group = factor(row_number() %% 3)) |>
  ...
  theme_classic() +
  scale_fill_manual(values = colors) +
  theme(axis.text.y = element_text(color = titles),
    axis.title.x = element_text(color = titles),
    axis.title.y = element_text(color = titles),
    plot.title = element_text(color = titles, face = "bold"),
    plot.subtitle = element_text(color = titles))+
  labs(
    title = 'Rising Google Search Terms & GDELT Event Source
      Levenshtein Similarity Score > 0.4',
    y = 'Term Count',
    x = 'Rising Term'
  )
...
sim_df |>
  ...
  labs(
    title = 'Rising Google Search Terms & GDELT Event Source
      JaroWinkler Similarity Score > 0.85',
    y = 'Term Count',
    x = 'Rising Term'
  )
...
sim_df |>
  ...
  theme(
    ...
    plot.subtitle = element_text(color = titles))+
  theme(axis.text.x=element_text(angle=50, hjust=1)) +
  theme(axis.text.y=element_text(angle=4)) +
  labs(
    title = 'Rising Google Search Terms & GDELT Event Source
      JaroWinkler Similarity Score > 0.85',
    y = 'GDELT Event Source',
    x = 'Rising Term',
    fill='JaroWinkler Score'
  )
```
Assessment:
